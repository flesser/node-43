...
function enqueueJobs() {
  ...
  // since CREST is limited to 10 requests per second.
  // (see https://forums.eveonline.com/default.aspx?g=posts&t=344803)
  //
...
function processJob(task, callback) {
  ...
  var options = {
    ...
    gzip: true,
    headers: {
     'User-Agent': 'Node43/2.2.0 (Git) <https://github.com/flesser/node-43/>'
    }
  };

  request(options, handleResponse.bind(null, task, callback));
  eventCenter.emit('crestRequest');
}
