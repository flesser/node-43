...
module.exports = function(relays) {
  ...
  for (var relay in this.relays) {
    process.stdout.write(new Date().toString() + ': Connecting to ' + relays[relay].underline + ':');
...
function watchConnection() {
  ...
  if((now.getTime() - this.lastMessage) > 10000) {
    console.log(new Date().toString(), 'Received no messages for 10 seconds. Reconnecting...')

    // Connect to the relays specified in the config file
    for (var relay in this.relays) {
      process.stdout.write(new Date().toString() + ': Reconnecting to ' + relays[relay].underline + ':');
